 ...
 */
function rewriteFormula(formula, cellMap) {
    // Build a combined replacement map sorted longest-key-first.
    const replacements = [];
    for (const [key, ref] of cellMap.fields)
        replacements.push([key, ref]);
    for (const [key, ref] of cellMap.synthetic)
        replacements.push([key, ref]);
 ...
 */
function fieldValueToCells(field, value, cellMap) {
    const pairs = [];
    ...
    if (field.type === 'multiselect') {
        ...
        if (lenRef)
            pairs.push([lenRef, selected.length]);
        const msField = field;
        for (const opt of msField.options) {
            const optRef = cellMap.synthetic.get(`${field.id}.${opt.value}`);
            if (optRef)
                pairs.push([optRef, selected.includes(opt.value) ? 1 : 0]);
        }
    }
 ...
 */
export async function computeCalculations(spec, values) {
    ...
    if (!hfCache || hfCache.specKey !== key) {
        ...
        const emptyData = Array.from({ length: totalRows }, () => [null]);
    ...
    for (const calc of spec.calculations) {
        try {
            ...
            hf.setCellContents({ sheet: sheetId, row: calcRowIndex, col: 0 }, [[`=${rewritten}`]]);
 ...
 */
export function computeCalculationsSync(spec, values) {
    if (!spec.calculations || spec.calculations.length === 0)
        return {};
    if (!hfCache || hfCache.specKey !== specKey(spec))
        return {};
...
export function invalidateCalcCache() {
    hfCache = null;
}
