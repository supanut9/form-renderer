    ...
    switch (field.type) {
        ...
        case 'number': {
            ...
            if (field.validation?.integer_only)
                s = s.int();
            const coerced = z.preprocess((v) => (v === '' || v == null ? undefined : Number(v)), field.required ? s : s.optional());
            return coerced;
        }
        case 'email': {
        ...
        case 'multiselect': {
            const validValues = field.options.map((o) => o.value);
            let s = z.array(z.string().refine((v) => validValues.includes(v), 'Invalid option'));
        ...
        case 'date': {
            ...
            const isoCheck = z
                .string()
                .refine((v) => /^\d{4}-\d{2}-\d{2}(T.*)?$/.test(v), 'Must be an ISO date (YYYY-MM-DD or YYYY-MM-DDTHH:mm:ss)')
                .refine((v) => !minDate || v >= minDate, minDate ? `Must be on or after ${minDate}` : '')
                .refine((v) => !maxDate || v <= maxDate, maxDate ? `Must be on or before ${maxDate}` : '');
...
export function buildZodSchema(spec, opts = {}) {
    const shape = {};
