...
// ── Internal helpers ──────────────────────────────────────────────────────────
function buildActionContext(values, calcResults, scoreResult) {
    return {
        ...values,
        ...calcResults,
...
function nextInOrder(spec, fromPageId) {
    const idx = spec.pages.findIndex((p) => p.id === fromPageId);
    if (idx < 0 || idx >= spec.pages.length - 1)
        return null;
 ...
 */
export function evaluatePageExitActions(spec, fromPageId, values, calcResults, scoreResult) {
    const defaultNext = nextInOrder(spec, fromPageId) ?? '__submit__';
    const actions = spec.actions ?? [];
    const relevantActions = actions.filter((a) => a.trigger === 'page_exit' && a.page_id === fromPageId);
